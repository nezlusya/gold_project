...
- Цель: таблица ods.gold_price_cbr (только последние 6 месяцев)
"""

SHORT_DESCRIPTION = "Загрузка котировок золота из MinIO (RAW) в PostgreSQL (ODS, 6 месяцев)"
...
with DAG(
    ...
    wait_for_raw_layer = ExternalTaskSensor(
        ...
        external_dag_id="newdag",  # ждём DAG, который грузит в MinIO
    ...
    start >> wait_for_raw_layer >> transfer >> end
